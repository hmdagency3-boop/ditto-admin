    ...
    <div className="flex flex-col gap-2">
      {visible.map(event => {
        ...
        return (
          <div
            key={event.id}
            className={`relative overflow-hidden rounded-xl border bg-gradient-to-l ${c.bg} ${c.border}`}
          >
            <div className="flex items-stretch">
              {/* Image — shown on the left (visual right in RTL) */}
              {hasImage && (
              ...
              <div className="flex flex-1 items-start gap-3 p-4 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    {hasImage && <Megaphone className={`h-4 w-4 shrink-0 ${c.text}`} />}
